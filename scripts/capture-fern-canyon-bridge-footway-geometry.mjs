...
const SOURCE_WAY_ID = 1481578623;
const SOURCE_WAY_VERSION = 1;
const NODE_IDS = [13588159627, 13588159632, 13588159633];
const FAR_ENDPOINT_NODE_ID = 13588159633;
...
const wayPayload = await fetchJson(
  wayVersionUrl,
  { headers: { "user-agent": "sandiegozoo-planner63-capture/1.0" } },
  "exact Fern Canyon bridge footway"
);
const way = wayPayload.elements?.find(
  element => element.type === "way" && element.id === SOURCE_WAY_ID
);
if (!way) throw new Error("Missing exact Fern Canyon bridge footway.");
if (
  way.version !== SOURCE_WAY_VERSION ||
  way.timestamp !== TARGET_TIMESTAMP ||
  JSON.stringify(way.nodes ?? []) !== JSON.stringify(NODE_IDS)
) {
  throw new Error("Fern Canyon bridge footway identity drifted.");
}
...
const overpass = await fetchJson(
  "https://overpass-api.de/api/interpreter",
  {
    ...
    headers: {
      ...
      "user-agent": "sandiegozoo-planner63-capture/1.0",
    },
    body,
  },
...
console.log("PLANNER63_CAPTURE_BEGIN");
...
console.log("PLANNER63_CAPTURE_END");
