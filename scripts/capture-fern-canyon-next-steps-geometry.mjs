...
const SOURCE_WAY_ID = 1481578624;
const SOURCE_WAY_VERSION = 1;
const NODE_IDS = [13588159634, 13588159633];
const FAR_ENDPOINT_NODE_ID = 13588159634;
...
const wayPayload = await fetchJson(
  wayVersionUrl,
  { headers: { "user-agent": "sandiegozoo-planner65-capture/1.0" } },
  "exact Fern Canyon next steps"
);
const way = wayPayload.elements?.find(
  element => element.type === "way" && element.id === SOURCE_WAY_ID
);
if (!way) throw new Error("Missing exact Fern Canyon next steps way.");
if (
  way.version !== SOURCE_WAY_VERSION ||
  way.timestamp !== TARGET_TIMESTAMP ||
  JSON.stringify(way.nodes ?? []) !== JSON.stringify(NODE_IDS)
) {
  throw new Error("Fern Canyon next steps identity drifted.");
}
...
const overpass = await fetchJson(
  "https://overpass-api.de/api/interpreter",
  {
    ...
    headers: {
      ...
      "user-agent": "sandiegozoo-planner65-capture/1.0",
    },
    body,
  },
...
console.log("PLANNER65_CAPTURE_BEGIN");
...
console.log("PLANNER65_CAPTURE_END");
